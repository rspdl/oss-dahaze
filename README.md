...
로그인은 `/login` 한 곳에서만 합니다. 루트(`/`)에는 화면이 없고 `/projects` 로 보냅니다.

**개발과 프로덕션이 같은 화면입니다.** 환경에 따라 열리고 닫히는 로그인은 없습니다 —
아이디·비밀번호는 어디서나 열려 있고, OAuth 는 자격증명을 넣은 서버에서만 버튼이 하나 더
붙습니다.

로컬에서는 **OAuth App 없이 시작합니다.** `/login` 의 가입 탭에서 아이디·표시 이름·비밀번호
(8자 이상)로 계정을 만들면 그대로 로그인됩니다. 계정을 여럿 만들면 프로젝트 공유나 권한
화면을 손으로 확인할 수 있습니다.

GitHub 으로도 들어오려면 OAuth App 을 만들어 `apps/api/.env` 에 자격증명을 넣습니다. 없으면
`github` 제공자가 등록되지 않고 `/api/auth/providers` 의 `providers` 가 빈 목록으로 옵니다.
