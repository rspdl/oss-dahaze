...
import { designBindingKey, type ActionOutcome, type DesignBinding, type ElementDesign, type ElementSelection, type PrototypeAction, type PrototypeMode, type SampleVariant, type SemanticProposal } from '@/features/mockup/prototype-contract'
...
import { handlerDisplay, prototypeOutcomesByScreen } from './prototype-outcomes'
...
function FlowView({ projectId }: { projectId: string }) {
  ...
  const prototype = useMemo(() => ({
    ...
    onAction: ({ outcome }: PrototypeAction) => {
      setActiveOutcome(outcome)
      if (outcome.targetScreenKey !== null && outcome.targetScreenKey !== undefined && graph.nodes.some((node) => node.id === outcome.targetScreenKey)) setSelectedId(outcome.targetScreenKey)
    },
  }), [dimensions, mode, sampleVariant, samples, selectedElement, effectiveDesign, selectedSampleIdByModel, experienceValues, setExperienceValues, setSelectedSampleIdByModel, graph.nodes])
  const prototypeForNode = useCallback((node: FlowNode) => ({ ...prototype, sourceHash: data.documentsByPath.get(node.screen.path)?.source_hash, outcomesByElementId: screenOutcomes[node.id] }), [data.documentsByPath, prototype, screenOutcomes])
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 rounded-control border bg-surface-raised/40 p-2 text-xs">
          <label>화면 <select aria-label="화면 찾기" disabled={selected === null} value={selected?.id ?? ''} onChange={(event) => setSelectedId(event.target.value)} className="max-w-48 rounded border bg-surface px-2 py-1 disabled:opacity-50">{graph.nodes.map((node) => <option key={node.id} value={node.id}>{node.screen.name}</option>)}</select></label>
          {environments.length > 0 ? <label>환경 <select value={environmentId} disabled={designDirty} title={designDirty ? '현재 환경의 배치 저장이 끝난 뒤 환경을 바꿀 수 있습니다.' : undefined} onChange={(event) => { const id = event.target.value; setEnvironmentId(id); setSelectedId(null); setSelectedElement(null); setProposal(null); setActiveOutcome(null); const next = environments.find((entry) => entry.id === id); if (next) setDimensions({ width: next.width, height: next.height }) }} className="rounded border bg-surface px-2 py-1 disabled:opacity-50"><option value="all">전체</option>{environments.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label> : null}
        ...
        {mode === 'experience' && activeOutcome?.handler ? <OutcomeNotice outcome={activeOutcome} onClose={() => setActiveOutcome(null)} /> : null}
...
function OutcomeNotice({ outcome, onClose }: { outcome: ActionOutcome; onClose: () => void }) {
  if (outcome.handler === null || outcome.handler === undefined) return null
  const popup = outcome.handler.kind === 'popup'
  return <div role={popup ? 'dialog' : 'status'} aria-label={`선언된 ${outcome.handler.kind} handler`} className="mt-2 flex items-center gap-2 rounded-control border border-accent/30 bg-accent-subtle px-3 py-2 text-xs"><p className="flex-1">{handlerDisplay(outcome.handler)}</p><button type="button" className="rounded border px-2 py-1" onClick={onClose}>닫기</button></div>
}
