...
export function HandoffInspector({ snapshot, compare }: { snapshot: ProjectSnapshotResponse; compare?: ProjectSnapshotResponse }) {
  ...
  const policyData = collectPolicies(compiled)
  const policyRows = policyData.rows
  const constraintsByField = new Map(policyData.fields.map((group) => [group.field.id, group.constraints.map((constraint) => `${constraint.left} ${constraint.operator} ${constraint.right}`)]))
  const board = collectBoard(compiled)
  const iaTree = buildIaTree(board)
  const orderedIa = orderIaNodes(iaTree)
...
function orderIaNodes(tree: ReturnType<typeof buildIaTree>) { const byId = new Map(tree.nodes.map((node) => [node.id, node])); const children = new Map<string, string[]>(); const childIds = new Set<string>(); for (const edge of tree.edges) { children.set(edge.source, [...(children.get(edge.source) ?? []), edge.target]); childIds.add(edge.target) } const ordered: typeof tree.nodes = []; const visit = (id: string) => { const node = byId.get(id); if (!node) return; ordered.push(node); for (const child of children.get(id) ?? []) visit(child) }; for (const node of tree.nodes) if (!childIds.has(node.id)) visit(node.id); return ordered }
