...
이 갈라짐의 대가는 **배포 방식을 바꿀 때 두 저장소를 함께 고쳐야 한다**는 것이다.
`rollout.sh` 의 인자가 바뀌면 `deploy.yml` 의 `send-command` 도 함께 바뀌어야 하고, 그
불일치를 CI 가 잡아 주지 않는다. 그래서 배포 경로를 건드리는 PR 은 짝이 되는 PR 을 함께 연다.
